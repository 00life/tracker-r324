...
function Login() {
    const { func_signin, func_snackbar, reference } = useAuth();
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
            try{
                const signin = await func_signin(email, password);
                if(signin.user) //used to throw an error
                signin?.user?.uid && navigate('/');
            }catch{func_snackbar(reference,'Failure to Sign In')}
    };
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <td colSpan={2}>
                                    <div style={{display:'flex', justifyContent:'center'}}>
                                        <input type="button" value="Create a New Account" data-dest='/signup' onClick={e=>handleButton(e)}
                                            style={{padding:'5px', fontSize:"15px", boxShadow:"1px 1px 4px 0px #8888"}}/>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colSpan={2}>
                                   <div style={{display:'flex', justifyContent:'center'}}>
                                    <input type="button" value="Forgot Login/Password?" data-dest="/forgot" onClick={e=>handleButton(e)}
                                        style={{padding:'5px', fontSize:"14px", boxShadow:"1px 1px 4px 0px #8888"}}/>
                                </div> 
                                </td>
                            </tr>
                        </tbody>
